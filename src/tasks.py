...
async def determine_monthly_winner():
    ...
    if pacific_time.day == 1:
        voice_channel = bot.get_channel(BotConstants.VOICE_CHANNEL_ID)
        ...
        if voice_channel and len(voice_channel.members) >= 1:
            ...
            text_channel = bot.get_channel(BotConstants.TEXT_CHANNEL_ID)
...
async def before_determine_daily_winner():
    ...
    target_time = datetime.replace(now, hour=WalkArgs.WINNER_HOUR, minute=WalkArgs.WINNER_MINUTE, second=0, microsecond=0)
    if now > target_time:
...
async def before_determine_monthly_winner():
    now = datetime.now()
    now = now.astimezone(pytz.timezone('US/Pacific'))
    target_time = datetime.replace(now, hour=WalkArgs.WINNER_HOUR, minute=WalkArgs.WINNER_MINUTE - 1, second=0, microsecond=0)
    if now > target_time:
        target_time += timedelta(days=1)
